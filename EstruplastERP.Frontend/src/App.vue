<script setup lang="ts">
import { ref, onMounted } from 'vue'
import { sesion } from './stores/sesion' // <--- 1. Importamos el Singleton de Sesión
import Login from './components/Login.vue' // <--- 2. Importamos el componente Login

// Importación de tus componentes del sistema
import FormularioProduccion from './components/FormularioProduccion.vue'
import ListaProduccion from './components/ListaProduccion.vue'
import GestionProductos from './components/GestionProductos.vue'
import IngresoStock from './components/IngresoStock.vue'
import EditorRecetas from './components/EditorRecetas.vue'
import ListaStock from './components/ListaStock.vue' 
import Administracion from './components/Administracion.vue'

const pestanaActual = ref('produccion')
const tablaHistorial = ref(null)

// 3. Al iniciar la app, intentamos recuperar la sesión guardada
onMounted(() => {
    sesion.recuperar()
})

function recargarTabla() {
    console.log("2. App: Aviso recibido. Intentando recargar tabla...")
    
    if (tablaHistorial.value) {
        console.log("3. App: Tabla encontrada. Ejecutando función interna.")
        tablaHistorial.value.cargarHistorial()
    } else {
        console.error("❌ ERROR: App no encuentra la tabla (tablaHistorial es null)")
    }
}
</script>

<template>
  <div class="contenedor-principal">
    
    <header>
      <div class="usuario-info">
          Hola, {{ sesion.usuario?.nombre || 'Modo Desarrollo' }} 
          
          <button @click="sesion.cerrar()" class="btn-salir">Cerrar Sesión</button>
      </div>

      <div class="logo-container">
        <img src="data:image/png;base64," alt="LOGO ESTRUPLAST" class="logo-img">
      </div>

      <nav class="menu">
        <button @click="pestanaActual = 'produccion'" :class="{ activo: pestanaActual === 'produccion' }">⚙️ Producción</button>
        <button @click="pestanaActual = 'productos'" :class="{ activo: pestanaActual === 'productos' }">📦 Productos</button>
        <button @click="pestanaActual = 'compras'" :class="{ activo: pestanaActual === 'compras' }">🚚 Ingreso Stock</button>
        <button @click="pestanaActual = 'recetas'" :class="{ activo: pestanaActual === 'recetas' }">🧪 Fórmulas</button>
        <button @click="pestanaActual = 'inventario'" :class="{ activo: pestanaActual === 'inventario' }">📊 Inventario</button>
        <button @click="pestanaActual = 'admin'" :class="{ activo: pestanaActual === 'admin' }">⚙️ Configuración</button>
      </nav>
    </header>

    <main>
      <div v-if="pestanaActual === 'produccion'" class="vista-produccion">
        <FormularioProduccion @guardado="recargarTabla" />
        <hr class="separador">
        <ListaProduccion ref="tablaHistorial" />
      </div>

      <IngresoStock v-if="pestanaActual === 'compras'" />
      <EditorRecetas v-if="pestanaActual === 'recetas'" />
      <GestionProductos v-if="pestanaActual === 'productos'" />
      <ListaStock v-if="pestanaActual === 'inventario'" />
      <Administracion v-if="pestanaActual === 'admin'" />
    </main>

  </div>
</template>

<style scoped>
/* Contenedor central (Layout) */
.contenedor-principal {
  font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
  max-width: 1200px;
  margin: 0 auto;
  padding: 20px;
  background-color: #fafafa;
  min-height: 100vh;
}

/* Cabecera */
header {
  margin-bottom: 30px;
  text-align: center;
  position: relative; /* Para posicionar el botón de salir */
}

/* Info de Usuario (Arriba a la derecha) */
.usuario-info {
    position: absolute;
    top: 0;
    right: 0;
    font-size: 0.9em;
    color: #555;
    background: white;
    padding: 8px 15px;
    border-radius: 20px;
    box-shadow: 0 2px 5px rgba(0,0,0,0.1);
    display: flex;
    align-items: center;
    gap: 10px;
}

.btn-salir {
    background: #c0392b;
    color: white;
    border: none;
    padding: 5px 10px;
    border-radius: 4px;
    cursor: pointer;
    font-size: 0.8em;
    transition: background 0.3s;
}
.btn-salir:hover { background: #a93226; }

.logo-img {
  max-height: 80px;
  margin-bottom: 15px;
}

/* Menú de Navegación */
.menu {
  display: flex;
  justify-content: center;
  gap: 10px;
  flex-wrap: wrap; 
  margin-top: 20px;
}

.menu button {
  padding: 10px 20px;
  border: none;
  background: #e0e0e0;
  cursor: pointer;
  font-size: 15px;
  border-radius: 6px;
  transition: all 0.2s;
  color: #555;
}

.menu button:hover { background: #d5d5d5; }

.menu button.activo {
  /* Nuevo Color de Marca para el elemento activo */
  background: #E67E22; 
  color: white;
  font-weight: bold;
  box-shadow: 0 2px 5px rgba(0,0,0,0.2);
}

/* Estilos específicos para la vista de Producción */
.vista-produccion {
  display: flex;
  flex-direction: column;
  gap: 30px; 
}

.separador {
  border: 0;
  border-top: 2px dashed #ccc;
  width: 100%;
}
</style>