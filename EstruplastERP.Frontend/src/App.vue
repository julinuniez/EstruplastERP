...
// Referencia para controlar la tabla desde aquí
const tablaHistorial = ref(null) // <--- CREAMOS ESTA REFERENCIA

// Función que ejecuta la recarga
function recargarTabla() {
    console.log("2. App: Aviso recibido. Intentando recargar tabla...") // <--- AGREGAR
    
    if (tablaHistorial.value) {
        console.log("3. App: Tabla encontrada. Ejecutando función interna.") // <--- AGREGAR
        tablaHistorial.value.cargarHistorial()
    } else {
        console.error("❌ ERROR: App no encuentra la tabla (tablaHistorial es null)") // <--- AGREGAR
    }
}
</script>
...
.contenedor-principal {
  ...
  max-width: 1200px; /* Ancho máximo para que no se estire en pantallas gigantes */
  margin: 0 auto;
  padding: 20px;
...
.menu {
  ...
  gap: 10px;
  flex-wrap: wrap; /* Para que no se rompa en celulares */
}
...
.vista-produccion {
  ...
  gap: 30px; /* Espacio vertical entre el formulario y la tabla */
}
