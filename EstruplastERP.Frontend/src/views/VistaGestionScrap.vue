...
// 👇 CAMBIO 1: Importamos la instancia segura en lugar de axios
...
const cargarClientes = async () => {
    try {
        // 👇 CAMBIO 2: Usamos api.get (ya no hace falta pasar headers manuales)
        ...
        if (e.code === "ERR_NETWORK" || e.response?.status >= 500) {
            errorConexion.value = "⛔ ERROR CRÍTICO: Sin conexión al servidor.";
        } else {
...
// Watcher para cargar productos (Igual pero con api)
watch(() => form.value.clienteId, async (nuevoId) => {
    ...
    try {
        ...
        if (Array.isArray(res.data)) {
            // Filtramos solo el SCRAP disponible de ese cliente
            listaScrap.value = res.data.filter((p: any) => 
                Number(p.clienteId) === Number(nuevoId) && 
                // Aseguramos que sea scrap (por nombre o propiedad) y tenga stock
                (p.esScrap === true || p.nombre.includes('SCRAP') || p.nombre.includes('SUCIO')) &&
                p.stockActual > 0
            );
        }
    } catch (e) {
...
const confirmarTransformacion = async () => {
    if (!form.value.clienteId || !form.value.scrapId) return alert("Selecciona Cliente y Material.");
    if (form.value.kilosEntrada <= 0 || form.value.kilosSalida <= 0) return alert("Ingresa los kilos.");
    if (form.value.kilosSalida > form.value.kilosEntrada) return alert("❌ Error: Salida mayor a Entrada.");
    
    // Validación de stock en front para feedback rápido
    if (scrapSeleccionado.value && form.value.kilosEntrada > scrapSeleccionado.value.stockActual) {
        return alert(`❌ Solo tienes ${scrapSeleccionado.value.stockActual} kg disponibles.`);
    }
    ...
    try {
        ...
        // 👇 CAMBIO 3: Enviamos y leemos la respuesta del backend
        const res = await api.post('/Produccion/transformar-scrap', payload);
        
        // Backend devuelve: { mensaje, producto, stockNuevo }
        const nombreGenerado = res.data.producto || 'Materia Prima';
        
        mensaje.value = `✅ ÉXITO: Se generó ${form.value.kilosSalida}kg de "${nombreGenerado}".`;
        
        // Limpiamos formulario
        form.value.kilosEntrada = 0;
        form.value.kilosSalida = 0;
        
        // Truco para refrescar la lista y que baje el stock visualmente
        const idActual = form.value.clienteId;
        form.value.clienteId = ''; 
        setTimeout(() => form.value.clienteId = idActual, 50);

    } catch (e: any) {
        const errorMsg = e.response?.data?.mensaje || e.response?.data || "Error en la transformación.";
